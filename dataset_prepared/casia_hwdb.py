...
this is a wrapper handle CASIA_HWDB dataset_prepared
...
def parse_example(record):
    ...
    # decode_raw将原来编码为字符串类型的变量重新变回来
    img = tf.io.decode_raw(features['image'], out_type=tf.uint8)
    # 用于数据类型的转变，不会改变原始数据的值还有形状
...
def load_val_ds():
    input_files = ['dataset_prepared/HWDB1.1tst_gnt.tfrecord']
    ds = tf.data.TFRecordDataset(input_files)
...
if __name__ == "__main__":
    ...
    if is_show_combine:
        ...
        print(res)
        cv2.imwrite('dataset/combined.png', combined)
        cv2.waitKey(0)
            # break
    else:
        i = 0
        for data in val_ds.take(36):
            ...
            print(label)
            cv2.imshow('rr', img)
            cv2.imwrite('dataset/{}.png'.format(i), img)
            i += 1
            cv2.waitKey(0)
            # break
